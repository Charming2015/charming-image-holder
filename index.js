...
function createSvgXml({width = 100, height = 100, bgColor = '#999', fColor = '#fff', text = undefined}){
	return `
		<svg xmlns="http://www.w3.org/2000/svg" height="${height}" width="${width}">
			...
			<text x="50%" y="50%" font-size="20" dy=".3em" fill="${fColor}" text-anchor="middle">${text || `${width}x${height}`}</text>
		</svg>
	`
}

app.get('/', function(req, res){
	const codeStr = code => `<code style="background-color: rgba(27,31,35,.05);border-radius: 3px;padding: 3px 8px;">${code}</code>`
	res.send(`<div>
		<h2>欢迎使用 charming-img-holder</h2>
		<h4>使用方法如下:</h4>
		<p>1. 传入宽高：${codeStr('http://img-holder.charming.run/200x100')}</p>
		${createSvgXml({ width: 200, height: 100 })}
		<p>(宽高一致，也可仅传${codeStr('/100')}, ${codeStr('http://img-holder.charming.run/100')})</p>
		${createSvgXml({ width: 100, height: 100 })}
		<p>2. 传入背景色、字体颜色${codeStr('/[width]x[height]/:bgColor/:fontColor')}，如：${codeStr('http://img-holder.charming.run/100/000/FF0000')}</p>
		${createSvgXml({ width: 100, height: 100, bgColor: '#000', fColor: '#FF0000' })}
		<p>3. 传入文字${codeStr('http://img-holder.charming.run/200?text=这是一段文字')}</p>
